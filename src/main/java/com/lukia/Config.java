...
public class Config {
    ...
    public static void main(String[] args) throws URISyntaxException {
        // Jarが読み込まれている階層へのPathを定義
        // 現在のクラスが含まれているJARファイルのURLを取得
        URL jarUrl = Config.class.getProtectionDomain().getCodeSource().getLocation();
        
        // URLからファイルパスを取得
        File jarFile = new File(jarUrl.toURI());

        // JARファイルが実行されているディレクトリを取得
        File jarDir = jarFile.getParentFile();
        Config.jarDirPath = jarDir.toPath();
    ...
    public Config() {
        this.dataDirectory = Config.jarDirPath.resolve("setting-web-server");
        instance = this;
    }
